...
class RIMAnalyzer:
    ...
    def _generate_result(self, events_data):
        # Тот же код генерации результата
        summary_parts = []
        has_errors = False
        ...
        for phase in ['A', 'B', 'C']:
            ...
            if len(events) > 10:
                ...
                count = len(events)
                summary_parts.append(f"Фаза {phase}: Перенапряжение (max {max_voltage:.0f}В) ({period}) - {count} событий")
        ...
        for phase in ['A', 'B', 'C']:
            ...
            if len(events) > 10:
                ...
                count = len(events)
                summary_parts.append(f"Фаза {phase}: Провал (min {min_voltage:.0f}В) ({period}) - {count} событий")
